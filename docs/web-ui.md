...
The tool list is a ceiling, not an active-tool inventory or configurable role profile. Unknown tools deny in the family run-tool controller, callers cannot widen its ceiling, and missing tool controls fail before prompting. Other direct/queued/add-on entry points and shared-resource policy still need integrated release verification. The panel clears on close/backgrounding/identity replacement and rejects unsupported activation claims.

This preview is English-only. Avatar changes, container destination assignment, attachments, streaming and pagination are unfinished. Session merge, purge and full archive backup are unavailable. Shared/provider/deployment editors, complete setting/add-on classification and per-user capability profiles are unfinished; single-user classic/visual Settings are unchanged. Chromium virtual-authenticator tests cover adding two independent keys; physical authenticator/device coverage is incomplete. The following sections describe the supported **single-user** UI.
