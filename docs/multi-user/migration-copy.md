...
To execute something from old history, the owner must review it and submit a new supported plain-text prompt through normal admission. Dismissal does not prefill, copy or submit content automatically. Original authorship remains intact. The browser shows the legacy hold separately, hides Retry, requires a checked dismissal and reuses the request key only for unchanged manual retries. Blur, navigation and identity replacement clear the controls; late responses cannot restore a previous account's state.
