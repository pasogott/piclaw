...
Choose **Inspect result** to load the stored result as plain text. Check the **Original conversation** and execution ID. An unsettled, expired-unsettled, expired or interrupted record cannot be published. `expired-unsettled` means the settlement deadline passed without a result; `expired` means internal maintenance recorded terminal expiry. `interrupted` means an admitted dispatcher could not settle and recorded a terminal failure. None of these states retries the model, proves the provider call stopped or undoes external work already performed. “Publication receipt recorded” means a receipt exists; confirming publication verifies the actual message before acknowledging a retry.
