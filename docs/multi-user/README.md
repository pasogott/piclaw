...
### Owner-scoped tool outputs

New family tool outputs bind the immutable owner, stable root and source branch IDs, creation-time chat JID and execution kind. Metadata, FTS snippets, full files, process cache entries and retention operations require that exact live execution scope. Another owner, another branch or a legacy unowned handle returns no record or content. Internal JID maintenance preserves access through the stable branch IDs while retaining the original JID as audit metadata.

Family files use deterministic owner/scope shards and restrictive file modes. Reads and cleanup reject path drift, traversal, symlink/hard-link substitution, file replacement and scope changes. Metadata and FTS chunks commit together; a login, account, ownership, mode or chat-context change during asynchronous work rolls back new records and suppresses late results. Family tool-result details expose the opaque handle, counts and preview but not the absolute storage path. Semantic-summary cache keys include the owner scope.

`search_tool_output` is part of the fixed family ceiling and remains subject to each account's tool-policy snapshot and live execution check. Legacy rows keep null scope fields and remain single-user-only. Version-three migration-copy preparation deletes copied legacy metadata and FTS rows with a count-only receipt; external legacy log files still require operator cleanup. The process-wide automatic retention timer remains single-user-only; family pruning is explicit within the active owner scope. Raw filesystem access and arbitrary installed extensions remain trusted and are not a confidentiality boundary.
