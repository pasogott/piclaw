...
| Model identity/memory (#1129/#1131) | Server identity before hydration, scoped model context and owner/family memory paths; mandatory fixed family run-tool ceiling | All direct/queued/delegate/side/Dream entry points and service grants; configurable capability profiles and shared-resource policy |
| Settings and isolation (#1130/#1132) | Own-account, owned-session lifecycle and account administration controls; read-only workspace/deployment/capability panel with scope descriptions | Container destination assignment, capability/preferences editors, complete setting/add-on classification and per-user container gateway/deployment |
...
The panel is read-only for administrators and members. No mode selector, per-user capability editor, preference editor or automatic restart is exposed. Complete setting/add-on scope classification, inheritance/effective preference values, capability enforcement across all direct/queued entry points, personal Dream and notification routing remain release work. Close, backgrounding, navigation and identity replacement clear displayed state; malformed policy responses fail closed. Existing single-user Settings are unchanged.
