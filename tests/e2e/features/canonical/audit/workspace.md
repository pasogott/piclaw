...
| `@ux-workspace-015` | VNC empty state / read-only / errors | `runtime/web/src/panes/vnc-pane.ts` — empty-state helper around `No saved VNC targets yet`, `readOnly`, `installVncClipboardControls`, `sendClientClipboardText`, `attachCanvasPointerHandlers`, `attachCanvasKeyboardHandlers`, error handlers for `vnc.error`, protocol failure, and session load failure | unavailable | No inspected automated VNC coverage was found. |
