...
export const EFFECTOR_CASE_CATALOGUE: readonly EffectorCaseCatalogueEntry[] = Object.freeze([
  ...
  },
  {
    ...
    requiredCases: numberedCases("EF-S06", [
      ...
      "malformed request fields and attempt numbers fail before boundary invocation",
      ...
      "malformed receipt and provider identity are rejected while certainty is derived",
      "mutable resolver bytes are defensively snapshotted before boundary observation",
    ]),
    faultPoints: [
      "before_effect", "effect_then_lost_acknowledgement", "acknowledgement_then_crash",
  ...
  },
  {
    ...
    requiredCases: numberedCases("EF-S08", [
      ...
      "arbitrary malformed runtime inputs resolve typed protected-payload errors",
    ]),
    faultPoints: [
      "before_effect", "effect_then_lost_acknowledgement", "duplicate_result", "delayed_or_late_result",
