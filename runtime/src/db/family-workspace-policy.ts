...
import { FAMILY_WEB_TOOLS, type FamilyWorkspacePolicy } from '../core/family-workspace-policy.js';
...
export function readFamilyWorkspacePolicy(database: Database, actor: AuthenticatedPrincipal): FamilyWorkspacePolicy {
  ...
  return database.transaction((): FamilyWorkspacePolicy => {
    ...
    return {
      ...
      tools: { policy: 'fixed-family-web-preview', configurable: false, allowed: [...FAMILY_WEB_TOOLS],
        scope: 'Ceiling for admitted plain-text family web turns. Active tools may be a smaller subset. Direct tools, queues, add-ons and transports still require integrated release verification.' },
      resources: [
      ...
      operations: [
        { name: 'File inspection', state: 'shared-read', detail: 'read, ls, find and grep operate on the shared filesystem; do not store private material there expecting user confinement.' },
        { name: 'Messages and session discovery', state: 'owner-scoped', detail: 'messages reads, session_status, session_control inspect/assess_stuck and chat directory are restricted to active owned sessions. Cross-session sends/control writes are disabled.' },
      ...
      settings: [
        ...
        { name: 'Tool activation and capability profile', scope: 'fixed web-turn policy', availability: 'Read-only ceiling; no per-user grants/denials editor yet.' },
