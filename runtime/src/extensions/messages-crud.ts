...
import { sanitizePublicInboundContentBlocks } from "../channels/web/messaging/content-block-safety.js";
...
function executePost(
  ...
  const mediaIds = Array.from(new Set((params.media_ids ?? []).filter((id) => Number.isInteger(id) && id > 0)));
  const contentBlocks = sanitizePublicInboundContentBlocks(params.content_blocks);
